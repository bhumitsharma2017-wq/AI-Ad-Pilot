      ...
      {scripts[activeScript] && (
        <div className="space-y-4">
          ...
          <div className="bg-red-50 border border-red-100 rounded-2xl p-5">
            ...
            <p className="text-red-900 font-semibold text-lg">{String(scripts[activeScript].hook)}</p>
          </div>
          ...
          <div className="bg-white rounded-2xl border border-gray-100 p-5">
            ...
            <p className="text-gray-800 text-sm leading-relaxed whitespace-pre-line">
              {String(scripts[activeScript].voiceover)}
            </p>
          </div>

          {/* Scenes */}
          {(scripts[activeScript].scenes as Record<string, unknown>[] || []).length > 0 && (
            <div className="bg-white rounded-2xl border border-gray-100 p-5">
              <h3 className="font-semibold text-gray-900 mb-3">Scene Breakdown</h3>
              <div className="space-y-3">
                {(scripts[activeScript].scenes as Record<string, unknown>[]).map((scene, i) => (
                  <div key={i} className="flex gap-3 p-3 bg-gray-50 rounded-xl">
                    <div className="flex-shrink-0">
                      <div className="w-8 h-8 rounded-full bg-red-100 flex items-center justify-center">
                        <span className="text-red-600 font-bold text-xs">{String(scene.scene_number)}</span>
                      </div>
                    </div>
                    <div className="flex-1 min-w-0">
                      <div className="text-xs text-gray-400 mb-1">{String(scene.duration_seconds)}s</div>
          ...
          <div className="grid md:grid-cols-2 gap-4">
            <div className="bg-white rounded-2xl border border-gray-100 p-5">
              ...
              <div className="text-lg font-bold text-brand-600">{String(scripts[activeScript].cta)}</div>
            </div>
            <div className="bg-white rounded-2xl border border-gray-100 p-5">
              <div className="text-xs font-bold text-gray-500 mb-2 uppercase tracking-wider">Production Notes</div>
              <p className="text-sm text-gray-600">{String(scripts[activeScript].storyboard_notes)}</p>
            </div>
          </div>
        </div>
      )}
